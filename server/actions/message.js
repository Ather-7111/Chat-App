...
exports.createMultipleMessages = async function (messageCreate) {
  try {
    ...
    const message = await prisma.message.create({
      data: {
        chatId: messageCreate[0].chatId,
        senderId: messageCreate[0].senderId,
        receiverId: messageCreate[0].receiverId,
        text: messageCreate[0].text,
      },
      include: {
        sender: true,
        conversation: true,
    ...
    console.log("mizaaj", { message:message, attachments:attachments });
